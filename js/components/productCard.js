...
function switchVariant(kode, idx) {
    ...
    if (specEl && v.spec) {
        ...
        // Reset toggle button
        const toggleBtn = specEl.parentNode?.querySelector(".spec-toggle-btn");
        if (toggleBtn && hidden.length > 0) {
            toggleBtn.dataset.open  = "false";
            toggleBtn.textContent   = `Lihat ${hidden.length} spesifikasi lainnya ▾`;
