...
const CartModal = {
    ...
    async _addToCart() {
        ...
        // Gunakan variant_kode sebagai kode unik supaya setiap variant
        // tersimpan sebagai item terpisah di cart.
        // Kalau variant_kode tidak ada (data lama), pakai kode + varian sebagai fallback
        const vKode     = v.variant_kode || v.kode || product.kode;
        const uniqueKey = (vKode && vKode !== product.kode)
            ? vKode
            : product.kode + (v.varian && v.varian !== "-" ? "_" + v.varian : "");

        await Cart.add({
            variant_kode: vKode,         // keyPath — unik per variant
            kode:         product.kode,
        ...
        return `
            <div class="draft-item">
                ...
                <div class="draft-item-info">
                    ...
                    <!-- Inline qty edit -->
                    <div class="draft-item-qty-edit">
                        ${["Carton","Inner","Pack","Pcs"].map(u => `
                            <div class="qty-edit-group">
                                <span class="qty-edit-label">${u}</span>
                                <input type="number"
